...
<a-form :layout="'inline'" ref="formRef" :model="formState" :rules="rules">
    ...
    <a-form-item label="  累计订单笔数满:" name="amount">
      <a-input-number
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    if (props && props["tagData"]) {
      formState.gapTime = (currentData && currentData.gapTime) || 0;
    ...
    const getDataString = (arr): Moment[] | string[] | string => {
      console.log(arr);
      if (arr && arr.length === 2) {
     ...
     gapTime: [
        {
          required: true,
          validator: () => {
            console.log(formState, 'string |string |')
            if (formState.type === 2) {
    ...
    watch(formState, () => {
      if (formState.type === 1) {
        if (formState.time?.length === 2) {
          assemble.ext.value  = ((getDataString(formState.time) as Moment[]).join(',') as string)
        }
      } else {
        // formState.gapType  是天月年
        assemble.ext.value =((getDays(formState.gapType) as number) * Number(formState.gapTime)).toString()
      }
      assemble.key = ''
